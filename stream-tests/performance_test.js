...
if (Meteor.isClient) {
	const done = async function(startTime, memoryStart, success, e, m) {
		console.log('success:', success, '/', e * m);
		console.log('time:', (new Date()) - startTime);
		const memoryEnd = await Meteor.callAsync('memory-sample');
		console.log('memory:', memoryReadle(memoryEnd - memoryStart), 'start:', memoryReadle(memoryStart), 'end:', memoryReadle(memoryEnd));
	};

	const test = async function(platform, e=2000, m=10) {
		const memoryStart = await Meteor.callAsync('memory-sample');
		await Meteor.callAsync('register', platform, e);
		...
		for (let i = 1; i <= e; i++) {
			for (let j = 1; j <= m; j++) {
				await Meteor[platform].emit(`event-${i}`, i, __meteor_runtime_config__);
			}
		}
	};
